...
while (<FILE1>){
	...
	if (exists $HASH{$NAME1}{$NAME2}){
		...
		$HASH{$NAME1}{$NAME2}[2] = $CHR;
		$HASH{$NAME1}{$NAME2}[3] = $POS1;
		$HASH{$NAME1}{$NAME2}[4] = $POS2;
...
foreach $KEY_NAME1 (sort {$a <=> $b} keys %HASH){
	foreach $KEY_NAME2 (sort {$a <=> $b} keys %{$HASH{$KEY_NAME1}}){
		...
		$CHR_REC = $HASH{$KEY_NAME1}{$KEY_NAME2}[2];
		$POS1_REC = $HASH{$KEY_NAME1}{$KEY_NAME2}[3];
		$POS2_REC = $HASH{$KEY_NAME1}{$KEY_NAME2}[4];
		if (($TAG_REC =~ /Already_Known_Fusion/) && ($TAG_REC !~ /Cancer_Gene_Involved/)){
			print FILE_PRINT "$CHR_REC\t$POS1_REC\t$KEY_NAME1\t$CHR_REC\t$POS2_REC\t$KEY_NAME2\t$TSR_REC\t$TSR_ONLY\t$TSR_ONLY\t$TAG_REC\n";
		}
			...
			print FILE_PRINT "$CHR_REC\t$POS1_REC\t$KEY_NAME1\t$CHR_REC\t$POS2_REC\t$KEY_NAME2\t$TSR_REC\t$TSR_ONLY\t$TSR_ONLY\n";
		}
	}
}
