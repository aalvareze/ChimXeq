...
while (<>){
	...
	if (($f1[1] ne $chr ||
		...
		($f1[8]<$genome_pos - $OFFSET && $f1[9]<$genome_pos - $OFFSET))
		|| ($f1[1] eq $chr_mate && (abs($genome_pos_mate - $f1[8])<100000))){
			next LINE;
	}
	...
	foreach my $i (@bres){
		my @f2 = split(/\s+/, $i);
		#print join("\t", @f1), "\n";
		if ($f2[2]>80 && $f2[1] eq $chr_mate && (abs($genome_pos_mate - $f2[8])<100000)){
				next LINE;
		}
	}
# print "$linea\n";
#}
	...
	$reads = "/home/xspuente/downloads/samtools-0.1.19/samtools faidx /data/human_ref/hg19_sanger/hg19.fa $chrom_mate:$start_mate-$end_mate";
	@output = qx{$reads};
	chomp($output[1]);
	if ($numero > 120){
		chomp($output[2]);
	...
	if (($f1[1] ne $chrom_38 ||
		...
		($f1[8]<$pos_38 - $OFFSET && $f1[9]<$pos_38 - $OFFSET))
		|| ($f1[1] eq $chrom_mate_38 && (abs($pos_mate_38 - $f1[8])<100000))){
			next LINE;
	}
	...
	foreach my $i (@bres){
		my @f2 = split(/\s+/, $i);
		if ($f2[2]>80 && $f2[1] eq $chrom_mate_38 && (abs($pos_mate_38 - $f2[8])<100000)){
				next LINE;
		}
	}
	print "$linea\n";
